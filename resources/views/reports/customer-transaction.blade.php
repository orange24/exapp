@extends('layouts.app')
@section('title', 'ธุรกรรมลูกค้า')
@section('content')
<div class="max-w-7xl mx-auto">
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        <h2 class="text-lg font-semibold text-gray-800 mb-4">รายงานธุรกรรมลูกค้า (Customer Transaction Report)</h2>
        <form method="GET" class="flex flex-wrap items-end gap-4">
            ...
            <div>
                ...
                <input type="text" name="search" value="{{ $search }}" placeholder="ชื่อ, เลข passport, สัญชาติ..." class="border border-gray-300 rounded-lg px-3 py-2 text-sm w-56">
            </div>
            <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700">แสดง</button>
            <button type="submit" formmethod="POST" formaction="{{ route('reports.customer-transaction.export') }}" class="bg-green-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-green-700 inline-flex items-center gap-1">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
                <thead><tr style="background:#0e513a;" class="text-white">
                    <th class="px-3 py-3 text-left font-semibold">ลูกค้า</th>
                    <th class="px-3 py-3 text-left font-semibold">เอกสาร</th>
                    <th class="px-3 py-3 text-left font-semibold">สัญชาติ</th>
                    ...
                    <th class="px-3 py-3 text-right font-semibold">รวม</th>
                </tr></thead>
                <tbody>
                    @forelse ($data as $idx => $d)
                    <tr class="{{ $idx % 2 === 0 ? 'bg-white' : 'bg-gray-50' }}">
                        <td class="px-3 py-2 font-medium">{{ $d->cust_name }}</td>
                        <td class="px-3 py-2 font-mono text-xs">{{ $d->id_number ?? '-' }}</td>
                        <td class="px-3 py-2">{{ $d->nationality ?? '-' }}</td>
